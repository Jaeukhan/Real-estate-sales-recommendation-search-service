...
//		serviceKey = URLDecoder.decode(serviceKey, "UTF-8");
    ...
    @ApiOperation(value = "학교 목록", response = List.class)
    @GetMapping(value = "/school/{sorting}/{gugunName}/{gugunCode}", produces = "application/json;charset=utf-8")
    public ResponseEntity<String> schoolList(@PathVariable("sorting") String sorting, @PathVariable("gugunName") String gugunName, @PathVariable("gugunCode") String gugunCode) throws IOException {
        logger.info("kinder - 호출");
        System.out.println(gugunName+" "+gugunCode);
        String serviceKey = "caa3a449c5c8411e910663f1df33cece";
        StringBuilder urlBuilder = new StringBuilder(
                " https://openapi.gg.go.kr/GgSchoolM");
